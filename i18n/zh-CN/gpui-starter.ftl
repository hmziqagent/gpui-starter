...
registration_form_label_variants-phone = 电话
...
registration_form_description_variants-phone = 格式：(555) 123-4567
registration_form_description_variants-website = 可选。您的个人或公司网站。

## FormAction

form_action-Submit = 创建账户
...
settings_test_action_notification = 测试操作按钮
settings_test_reply_notification = 测试回复
settings_test_background_notification = 测试后台策略
settings_action_notification_body = 请在通知中选择一个操作。选择的操作会写入日志。
settings_reply_notification_body = 请直接在通知中回复。输入的文本会写入日志。
settings_background_notification_body = 这会测试后台通知投递和降级回退行为。
...
form_page_title = 创建账户
form_page_subtitle = 由 gpui-form + koruma 验证驱动
form_page_success = 账户创建成功！请检查您的邮箱进行验证。
form_agree_terms = 我同意条款和条件
form_submit = 创建账户
form_reset = 重置
